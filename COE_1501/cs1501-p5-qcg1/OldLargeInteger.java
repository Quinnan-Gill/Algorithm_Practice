...
public class LargeInteger {
	...
	private LargeInteger multiply(byte[] a, byte[] b) {
		assert a.length == b.length;

		if (a.length == 1 && b.length == 1) {
			return byteMult(a[0], b[0]);
		}
		...
		byte[] a_high = new byte[mid];
		byte[] a_low = new byte[a.length - mid];

		byte[] b_high = new byte[mid];
		byte[] b_low = new byte[a.length - mid];
		...
		LargeInteger m1 = multiply(a_high, b_high);
		LargeInteger m4 = multiply(a_low, b_low);

		byte[] m5_x = new LargeInteger(a_high).add(new LargeInteger(a_low)).getVal();
		byte[] m5_y = new LargeInteger(b_high).add(new LargeInteger(b_low)).getVal();

		LargeInteger m5 = multiply(m5_x, m5_y).subtract(m1).subtract(m4);
		return shift(m1, n).add(shift(m5, n/2)).add(m4);
	}

	private LargeInteger shift(LargeInteger m, double n) {
		if (n % 1 != 0) {
			m = half_shift(m);
		}
		...
		byte[] res = new byte[a.length + (int) n];
	...
	private LargeInteger half_shift(LargeInteger m) {
		byte[] a = m.getVal();
		int carry = 0;
		byte[] res = new byte[a.length + 1];

		for(int i=a.length - 1; i >= 0; i--) {
			carry |= (((int) a[i] & 0xFF) << 4);
			res[i+1] = (byte) (carry & 0xFF);
            carry = carry >>> 8;
		}

		res[0] = (byte) (carry & 0xFF);

		return new LargeInteger(res);
	}

	private LargeInteger byteMult(byte a, byte b) {
		boolean neg = false;

		int carry = 0;
		byte [] result = new byte[2];
		carry = ((int) a & 0xFF) * ((int) b & 0xFF);
		result[1] = (byte) (carry & 0xFF);
		carry = carry >>> 8;
